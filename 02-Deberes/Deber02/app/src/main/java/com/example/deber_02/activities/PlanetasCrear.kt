...
class PlanetasCrear : AppCompatActivity() {
    ...
    private fun crearNuevoPlaneta(nombre: String, diametro: Double, tieneSatelites: Boolean, fechaDescubrimiento: Date): Boolean {
        val id = BDatosMemoriaPlaneta.idsPlaneta + 1
        val nuevoPlaneta = BPlaneta(id, nombre, diametro, tieneSatelites, fechaDescubrimiento)
        return BDatosMemoriaPlaneta.crearPlaneta(nuevoPlaneta).also {
            if (it) BDatosMemoriaPlaneta.idsPlaneta += 1
        }
    }
    ...
    private fun actualizarPlanetaExistente(planetaId: Int, nombre: String, diametro: Double, tieneSatelites: Boolean, fechaDescubrimiento: Date): Boolean {
        val nuevoPlaneta = BPlaneta(planetaId, nombre, diametro, tieneSatelites, fechaDescubrimiento)
        return BDatosMemoriaPlaneta.actualizarPlaneta(planetaId, nuevoPlaneta)
    }
